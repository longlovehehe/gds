 ...
 function makeCharts(data){
           chart = AmCharts.makeChart("{$data_type}", {
                type: "serial",
                dataProvider: data,
                categoryField: "date",
                startDuration: 1,
...
{if $data_type=='_term_type'}
    ...
    <div class="freezediv" id="_term_type_data_table" style="overflow-y: scroll;overflow-x: hidden; width: 100%;height: 399px;display: none;">
        <table  id="freezedivTable16" class="base full">
           ...
           {foreach item=item from=$type_list}
            <tr>
                <td width="195px">{$item.create_time}</td>
    ...
    makeCharts_term(eval($("input[name=json_mdt16]").val()));
        ...
        function term_type_charts_table(obj){
            ...
            layer.open({
                ...
                fix: false, //不固定
                maxmin: false,
                content: "?m=report&a=next_info_charts&date_type={$change_time['check_date']}&check_date={$check_date}&data_type=_term_type_data&table_type=term_type_data_table&index=666&stackType=none&total=total&time="+obj.item.dataContext.date
            });
        }
