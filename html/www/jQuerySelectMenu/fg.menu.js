...
function Menu(caller, options) {
	...
	this.create = function () {
		...
		var num = -1;
		$("ul.fg-menu").children().each(function () {
			num++;
			if ($(this).children().attr("class").indexOf("next_list") >= 0 || num <= lineNum || $(this).children().attr("class").indexOf("prev_list") >= 0) {
		...
		for (var i = 0; i < lineNum; i++) {
			$("ul.fg-menu").children().eq(i).removeClass("none");
		}
	};
	var nu = 0;
	this.chooseItem = function (item) {
		...
		$("ul.fg-menu").children().each(function () {
			...
			if ($(this).children().attr("class").indexOf("next_list") >= 0 || $(this).children().attr("class").indexOf("prev_list") >= 0) {
				flag = false;
			} else {
				$(this).addClass("none");
			}
		...
		var maxpage = Math.ceil(num / lineNum);
		var page=$("input[name=page]").val();
		// edit this for your own custom function/callback:
		if ($(item).attr("class").indexOf("next_list") >= 0) {
			if ( page>= maxpage - 1) {
				nu = 0;
			} else {
				nu++;
			}
			$("input[name=page]").val(nu);

		} else if ($(item).attr("class").indexOf("prev_list") >= 0) {
			if (page <= 0) {
				nu = maxpage - 1;
			} else {
				nu--;
			...
			menu.kill();
			$('input[name=ep_id]').val($(item).attr("ag_number"));
			$('#remote_input').val($(item).text());
			location.href = $(item).attr('href');

		}
		for (var i = nu * lineNum; i < (nu + 1) * lineNum; i++) {
			$("ul.fg-menu").children().eq(i).removeClass("none");
		}
	};
}
...
//		case 'bottom': yVal = dims.refH;
	...
	if (options.positionOpts.directionH == 'left') {
		...
		el.css({right: 'auto', left: xVal - 191});
		if (options.positionOpts.detectH && !fitHorizontal(el)) {
			el.css({left: 'auto', right: xVal});
		}
	}
